...
def start(request, app, model, object_id):
    ...
    if request.POST.get("post"):
        ...
        for user in User.objects.filter(id__in=val):
            todo = TodoList.objects.create(inst=workflow_inst, node=next_node, user=user, app_name=app,
                                           model_name=model)
            ...
            try:
                if delete_instance:
                    ...
                    if is_stop_node:
                        ...
                        for user in User.objects.filter(id__in=val):
                            todo = TodoList.objects.create(inst=workflow_instance, node=next_nodes[0], user=user,
                                                           app_name=app, model_name=model)
                        if current_tmp.status_field and current_tmp.status_value:
                            try:
                                ...
                                pass
                    History.objects.create(inst=workflow_instance, user=request.user, pro_type=int(operation),
                                           memo=memo, node=current_tmp)
